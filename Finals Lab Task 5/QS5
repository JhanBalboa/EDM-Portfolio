...
INSERT INTO vendorsACode VALUES 
(615, 'AC'), 
(904, 'SF'), 
(901, 'MAG');
...
CREATE VIEW view_total_price_by_vendors AS
SELECT P_CODE, P_DESCRIPT, P_ONHAND, P_PRICE,
       (P_ONHAND * P_PRICE) AS TOTAL_PRICE,
       V_CODE
FROM Products
WHERE V_CODE IN (21344, 23119, 24288);

SELECT * FROM view_total_price_by_vendors;
...
BEGIN
  ...
  SELECT GROUP_CONCAT(P_DESCRIPT SEPARATOR ', ')
  INTO result
  FROM Products p
  JOIN vendors v ON p.V_CODE = v.V_CODE
